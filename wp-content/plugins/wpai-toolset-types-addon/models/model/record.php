 ...
 */
class PMTI_Model_Record extends PMTI_Model {
	/**
	 * Initialize model
	...
	/**
	 * @see PMTI_Model::getBy()
	 * @return PMTI_Model_Record
	 */
	 ...
	 * @return PMTI_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMTI_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = [];
		...
		return $related instanceof PMTI_Model_List ? $related->convertRecords() : $related;
	}
