 ...
 */
class PMGI_Model_Record extends PMGI_Model {
	/**
	 * Initialize model
	...
	/**
	 * @see PMGI_Model::getBy()
	 * @return PMGI_Model_Record
	 */
	 ...
	 * @return PMGI_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMGI_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = [];
		...
		return $related instanceof PMGI_Model_List ? $related->convertRecords() : $related;
	}
