 ...
 */
abstract class PMGI_Controller {
	...
	protected function render($viewPath = null) {
		...
		if (is_null($viewPath)) {
			...
			$viewPath = str_replace('_', '/', preg_replace('%^' . preg_quote(PMGI_Plugin::PREFIX, '%') . '%', '', strtolower($trace[1]['class']))) . '/' . $trace[1]['function'];
		}
		// append file extension if not specified
		if ( ! preg_match('%\.php$%', $viewPath)) {
			$viewPath .= '.php';
		}
		$filePath = PMGI_Plugin::ROOT_DIR . '/views/' . $viewPath;
		if (is_file($filePath)) {
			extract($this->data);
			include $filePath;
		} else {
	...
	protected function error($msgs = NULL) {
		...
		$viewPathRel = str_replace('_', '/', preg_replace('%^' . preg_quote(PMGI_Plugin::PREFIX, '%') . '%', '', strtolower(get_class($this)))) . '/error.php';
		if (is_file(PMGI_Plugin::ROOT_DIR . '/views/' . $viewPathRel)) { // if calling controller class has specific error view
