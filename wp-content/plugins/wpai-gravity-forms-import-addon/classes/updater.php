...
if( ! class_exists('PMGI_Updater') ) {
    ...
    class PMGI_Updater {
        ...
        public function plugin_row_meta( $links, $file ) {
            if ( $file == $this->name ) {
                $row_meta = [
                    'changelog'    => '<a href="' . admin_url( 'plugin-install.php?tab=plugin-information&plugin=wpai-gravity-forms-add-on&section=changelog&TB_iframe=true&width=600&height=800' ) . '" class="thickbox open-plugin-details-modal" title="' . esc_attr( __( 'View WP All Import - Gravity Forms Add-On Pro Changelog', PMGI_Plugin::TEXT_DOMAIN ) ) . '">' . __( 'Changelog', PMGI_Plugin::TEXT_DOMAIN ) . '</a>',
                ];
