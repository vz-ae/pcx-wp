	...
	<div class="wpallimport-content-section">
		...
		<?php if ( "new" == $post['wizard_type']): ?>	
			<?php 
				if ( ! $isWizard ) {
					?>
					<h3><?php _e('Record Matching', 'wp_all_import_gf_add_on'); ?></h3>
					<?php 
				} else {
					if ( ! empty(PMXI_Plugin::$session->deligate) and PMXI_Plugin::$session->deligate == 'wpallexport' ) {
						?>
						<h3 style="padding-left:0;"><?php _e('Choose how exported data will be re-imported.','wp_all_import_gf_add_on');?></h3>
						<?php 
					} else {
						?>
						<h3 style="padding-left:0;"><?php printf(__('WP All Import will create new %s for each unique record in your file.','wp_all_import_gf_add_on'), $custom_type->labels->name);?></h3>
						<?php 
					}
				} 
			?>	
					...
					<table class="form-table" style="max-width:none;">
						<tr>
							<td>
								...
								<?php if ( ! $isWizard ):?>
								<h4><?php printf(__('WP All Import will associate records in your file with %ss it has already created from previous runs of this import basis on the Unique Identifier.','wp_all_import_gf_add_on'), $custom_type->labels->singular_name);?></h4>
								<?php endif; ?>
		...
		<?php else: ?>
			<?php if ( ! $isWizard ):?>
				<h3><?php _e('Record Matching', 'wp_all_import_gf_add_on'); ?></h3>
			<?php else: ?>
				<h3 style="padding-left:0;"><?php printf(__('WP All Import will merge data into existing %s.','wp_all_import_gf_add_on'), $custom_type->labels->name);?></h3>
			<?php endif; ?>
			</div>
			<div class="wpallimport-collapsed-content" style="padding: 0;">
				<div class="wpallimport-collapsed-content-inner">
						...
						<tr>
							<td>						
								<div class="input">
									...
									<h4><?php printf(__('Records in your file will be matched with %s on your site based on...', 'wp_all_import_gf_add_on' ), $custom_type->labels->name);?></h4>
                                    <div class="input">
                                        <input type="radio" id="duplicate_indicator_entity_id" class="switcher" name="duplicate_indicator" value="pid" checked="checked"/>
                                        <label for="duplicate_indicator_entity_id"><?php _e('Entry ID', 'wp_all_import_gf_add_on' )?></label><br>
                                        <span class="switcher-target-duplicate_indicator_entity_id" style="vertical-align:middle; padding-left:17px;">
