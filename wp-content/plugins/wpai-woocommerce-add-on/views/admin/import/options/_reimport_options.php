...
<?php
    ...
    if (!empty($post['custom_type'])){
        ...
        if ($post['custom_type'] == 'shop_order') {
...
<div class="input">
	...
	<label for="create_new_records"><?php printf(__('Create new %s from records newly present in your file', PMWI_Plugin::TEXT_DOMAIN), $cpt_name); ?></label>
	<?php if ( ! empty(PMXI_Plugin::$session->deligate) and PMXI_Plugin::$session->deligate == 'wpallexport' ): ?>
	<a href="#help" class="wpallimport-help" title="<?php printf(__('New %s will only be created when ID column is present and value in ID column is unique.', PMWI_Plugin::TEXT_DOMAIN), $cpt_name) ?>" style="top: -1px;">?</a>
	<?php endif; ?>
</div>
<div class="input">
	...
	<label for="is_not_keep_former_posts"><?php printf(__('Update existing %s with changed data in your file', PMWI_Plugin::TEXT_DOMAIN), $cpt_name); ?></label>
	...
	<div class="switcher-target-is_not_keep_former_posts" style="padding-left:17px;">
		...
		<div class="switcher-target-update_choosen_data"  style="padding-left:27px;">
			...
			<div class="input">
				...
				<a href="#help" class="wpallimport-help" style="position: relative; top: -2px;" title="<?php printf(__('Hint: uncheck this box to keep trashed %s in the trash.', PMWI_Plugin::TEXT_DOMAIN), $cpt_name); ?>">?</a>
			</div>
			<div class="input">
