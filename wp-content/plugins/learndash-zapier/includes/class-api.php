...
class LearnDash_Zapier_Api {
    ...
    public static function init() {
        ...
        // Create an endpoint
        add_action( 'init', array( __CLASS__, 'api_endpoint' ), 1 );
        ...
        // Filter payload
        add_filter( 'learndash_zapier_api_payload', [ __CLASS__, 'filter_api_payload' ], 10, 1 );
        add_filter( 'learndash_zapier_api_action_payload', [ __CLASS__, 'filter_api_payload' ], 10, 1 );
    }
        ...
        if ( isset( $request['action'] ) ) {
            switch ( $request['action'] ) {
                ...
                case 'get_sample':
                    ...
                    $response = [ self::get_trigger_sample( $request['trigger'], array_merge( $payload, $request ) ) ];
                    self::bail( $response );
                    break;
    ...
    public static function init_trigger( $trigger, $payload = array() ) {
        ...
        $hook_urls = [];
        $subscriptions = array_filter( $subscriptions, function( $subscription ) use ( $trigger, &$hook_urls, $payload ) {
    ...
    public static function init_trigger_course_completed( $data ) {
        ...
        $payload = array(
            ...
            'course_info' => self::get_response( 'course_info', [ 'user_id' => $data['user']->ID, 'course_id' => $data['course']->ID ] ),
     ...
     */
    public static function get_lessons_list( $args = [] ) {
     ...
     */
    public static function get_topics_list( $args = [] ) {
     ...
     */
    public static function get_quizzes_list( $args = [] ) {
     ...
     */
    public static function get_trigger_sample( $trigger, $args = [] ) {
        switch ( $trigger ) {
     ...
     */
    public static function get_object_sample( $object, $args = [] ) {
        switch ( $object ) {
            case 'user':
                // User
            ...
            case 'course':
                ...
                $courses = get_posts( array(
                    'post_type' => 'sfwd-courses',
                    'posts_per_page' => 1,
                    'include' => ! empty( $args['courses_ids'] ) ? $args['courses_ids'] : ( ! empty( $args['course_id'] ) ? [ $args['course_id'] ] : [] ),
                    'orderby' => 'rand',
                ) );
            ...
            case 'lesson':
                ...
                $lessons = get_posts( array(
                    'post_type' => 'sfwd-lessons',
                    'posts_per_page' => 1,
                    'include' => ! empty( $args['lessons_ids'] ) ? $args['lessons_ids'] : ( ! empty( $args['lesson_id'] ) ? [ $args['lesson_id'] ] : [] ),
                    'orderby' => 'rand',
                ) );
            ...
            case 'topic':
                ...
                $topics = get_posts( array(
                    'post_type' => 'sfwd-topic',
                    'posts_per_page' => 1,
                    'include' => ! empty( $args['topics_ids'] ) ? $args['topics_ids'] : ( ! empty( $args['topic_id'] ) ? [ $args['topic_id'] ] : [] ),
                    'orderby' => 'rand',
                ) );
            ...
            case 'group':
                ...
                $groups = get_posts( array(
                    'post_type' => 'groups',
                    'posts_per_page' => 1,
                    'include' => ! empty( $args['groups_ids'] ) ? $args['groups_ids'] : ( ! empty( $args['group_id'] ) ? [ $args['group_id'] ] : [] ),
                    'orderby' => 'rand',
                ) );
            ...
            case 'group_certificate_link':
                $sample = self::get_response( 'group_certificate_link', add_query_arg( [ 'group_id' => 1, 'user' => 1, 'cert-nonce' => 'abc123' ], home_url( '/sample-certificate/' ) ) );
                break;

            case 'course_certificate_link':
                $sample = self::get_response( 'course_certificate_link', add_query_arg( [ 'course_id' => 1, 'user' => 1, 'cert-nonce' => 'abc123' ], home_url( '/sample-certificate/' ) ) );
                break;
    ...
    {
        $courses = get_posts( [ 'post_type' => 'sfwd-courses', 'posts_per_page' => 10 ] );
        ...
        if ( empty( $user_id ) ) {
            $user_id = get_users( [ 'number' => 1, 'role' => 'administrator' ] )[0]->ID;
        }
            ...
            if ( is_array( $quiz_result ) ) {
                ...
                if ( $count > 0 ) {
                    ...
                    if ( ! empty( $quiz_result[ $key ] ) && is_array( $quiz_result[ $key ] ) ) {
                        ...
                        if ( 
                            count( array_diff( $result_keys, $default_keys ) ) < 1 
