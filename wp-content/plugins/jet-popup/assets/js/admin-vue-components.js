...
let jetPopupSettinsMixin = {
	...
	methods: {
		saveOptions: function() {
			...
			wp.apiFetch( {
				method: 'post',
				path: window.jetPopupSettingsConfig.saveSettingsPath,
				data: this.preparedOptions,
			} ).then( ( response ) => {
...
Vue.component( 'jet-popup-general', {
	template: '#jet-dashboard-jet-popup-general',
	...
	computed: {
		cacheTimeoutOptions: function () {
			return window.jetPopupSettingsConfig.cacheTimeoutOptions
		}
	},
	...
	methods: {
		clearCache: function() {
			...
			wp.apiFetch( {
				method: 'post',
				path: window.jetPopupSettingsConfig.clearPopupCachePath,
				data: {},
			} ).then( ( response ) => {
