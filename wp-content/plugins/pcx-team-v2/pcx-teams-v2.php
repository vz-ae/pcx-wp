...
function persons_list(){
    ...
    if($coach){
        ...
        $coach_calender_link = $coach['coach_calender'];
    ...
    if($tl){
        $tl_name = $tl['tl_name'];
        $tl_title = $tl['tl_title'];
        $tl_phone = $tl['tl_phone'];
        $tl_email = $tl['tl_email'];
        $tl_calender_link = $tl['tl_calendar'];
        $tl_intro = $tl['intro'];
    ...
    if($mca){
        ...
        $mca_title = $mca['mca_title'];
        $mca_phone = $mca['mca_phone'];
        $mca_email = $mca['mca_email'];
        $mca_calender_link = $mca['mca_calendar'];
        $mca_intro = $mca['intro'];
    ...
    if($managing_broker_checkbox){
        ...
        $managing_broker_title = $managing_broker['managing_broker_title'];
    ...
    if($services_manager_checkbox){
        ...
        $services_manager_title = $services_manager['services_manager_title'];
        ...
        <div class="vendors-list persons-list">
            ...
            <div class="single-vendor">
                    <div class="single-vendor-header">
                        ...
                        <div class="vendor-basic-info">
                            <?php if($coach_name){ ?>
                                <div class="vendor-company"><?php echo $coach_title; ?></div>
                            <?php } ?>
                        ...
                        <div class="vendor-contact-info-wrap">
                            ...
                            if($coach_calender_link){ ?>
                                <div class="single-contact-info">
                                    ...
                                    <div class="contact-info-detail">
                                        <?php echo "<a target='_blank' href='$coach_calender_link'>Book Here</a>"; ?>
                                    </div>
                                </div>
                            <?php } ?>
                            <?php if($coach_intro){ ?>
            ...
            <div class="single-vendor">
                    <div class="single-vendor-header">
                        ...
                        <div class="vendor-basic-info">
                            <?php if($mca_name){ ?>
                                <div class="vendor-company"><?php echo $mca_title; ?></div>
                            <?php } ?>
                    ...
                    <div class="single-vendor-details">
                        <div class="vendor-contact-info-wrap">
                            ...
                            <?php }
                            if($mca_calender_link){ ?>
                                <div class="single-contact-info">
                                    <div class="contanct-info-icon">
                                        <i class="fa fa-globe" aria-hidden="true"></i>
                                    </div>
                                    <div class="contact-info-detail">
                                        <?php echo "<a target='_blank' href='$mca_calender_link'>Book Here</a>"; ?>
                                    </div>
                                </div>
                            <?php } ?>
                            <?php if($mca_intro){ ?>
            ...
            <div class="single-vendor">
                    <div class="single-vendor-header">
                        ...
                        <div class="vendor-basic-info">
                            <?php if($tl_name){ ?>
                                <div class="vendor-company"><?php echo $tl_title; ?></div>
                            <?php } ?>
                        ...
                        <div class="vendor-contact-info-wrap">
                            ...
                            if($tl_calender_link){ ?>
                                <div class="single-contact-info">
                                    ...
                                    <div class="contact-info-detail">
                                        <?php echo "<a target='_blank' href='$tl_calender_link'>Book Here</a>"; ?>
                                    </div>
                                </div>
                            <?php } ?>
                            <?php if($tl_intro){ ?>
                    ...
                    <div class="single-vendor-header">
                        ...
                        <div class="vendor-basic-info">
                            <?php if($managing_broker_name){ ?>
                                <div class="vendor-company"> <?php echo $managing_broker_title; ?> </div>
                            <?php } ?>
                        ...
                        <div class="vendor-basic-info">
                            <?php if($services_manager_name){ ?>
                                <div class="vendor-company">
                                   <?php echo $services_manager_title; ?>
                                    </div>
                            <?php } ?>
...
function pcx_is_user_logged_in(){
   ...
   if ( is_user_logged_in() ) {
    ...
    /* For Test Mode 
    echo "<h6 style='clear:both;' >*** is_offic_id_set = " . $is_offic_id_set . " ***</h6>";
    */
...
function pcx_user_profile_fields2($user){
    global $pagenow;
    $user_office_id = null;
    if($pagenow !== 'user-new.php'){
        $user_office_id2 = get_user_meta( $user->ID, 'pcx_office2', true );
    }
  ?>
    <table class="form-table">
        <tr>
            <th><label for="pcx_office2">Select Agent Type</label></th>
            <td>
                <select name="pcx_office2" id="pcx_office2">
					<option value="">Select Agent Type</option>
                    <option value="PXC Agent" <?php if($user_office_id2=='PXC Agent'){ echo 'selected'; } ?> > PXC Agent</option>
					<option value="M-60" <?php if($user_office_id2=='M-60'){ echo 'selected'; } ?>>M-60</option>
					<option value="Top 20" <?php if($user_office_id2=='Top 20'){ echo 'selected'; } ?>>Top 20</option>
                </select>
            </td>
        </tr>
    </table>
...
add_action( 'show_user_profile', 'pcx_user_profile_fields2' );
add_action( 'edit_user_profile', 'pcx_user_profile_fields2' );
add_action( "user_new_form", "pcx_user_profile_fields2" );
   
function save_pcx_user_profile_fields2($user_id){
    if(isset($_POST['pcx_office2'])){
	 update_usermeta($user_id, 'pcx_office2', $_POST['pcx_office2']);	
	}
}
add_action('user_register', 'save_pcx_user_profile_fields2');
add_action('profile_update', 'save_pcx_user_profile_fields2');

/* ===================== FW CODE ==================== */

// Get Current User Office ID
add_shortcode( 'get-current-user-office', 'get_current_user_office' );

// Shortcode For displaying Calendar V2
add_shortcode( 'office-field', 'get_office_field' );
function get_office_field($atts){
    $args = shortcode_atts( array(
        'field' => null
    ), $atts);
    $user_office_id = get_current_user_office();
    ...
    if($field === 'post_title'){
        $value = get_the_title($user_office_id);
        return $value;
    }

    if ($field === 'post_author') {
        $value = get_post_field( 'post_author', $user_office_id);
        return $value;
    }

    $value = get_field($field, $user_office_id);
    return $value;
}

// Shortcode For displaying Calendar V2
add_shortcode( 'pcx-office-calender-v2', 'pcx_office_calender_v2' );
function pcx_office_calender_v2(){
    ob_start();
    $user_id = get_current_user_id();
    $user_office_id = get_user_meta( $user_id , 'pcx_office', true );
    if($user_office_id && $user_office_id!=""){ // If User has Office ID
        $calender_link = get_field('pcx_office_calender',$user_office_id);
        $calender_iframe = "<iframe src='$calender_link&mode=AGENDA&bgcolor=%23ffffff&color=%23AA5A00&showTitle=1&showDescription=0&showNav=0&showDate=1&showPrint=0&showTabs=0&showCalendars=0&showTz=0' style='border-top:solid 1px #DEDEDE' width='auto' height='400' frameborder='0' scrolling='yes'></iframe>";
        echo $calender_iframe;
    }else{ // Otherwise
        echo 'No Office Linked With Your Profile';
    }
...
// Shortcode For displaying Calendar V2 - MONTH
add_shortcode( 'pcx-office-calender-v2-month', 'pcx_office_calender_v2_month' );
function pcx_office_calender_v2_month(){
    ob_start();
    $user_id = get_current_user_id();
    $user_office_id = get_user_meta( $user_id , 'pcx_office', true );
    if($user_office_id && $user_office_id!=""){ // If User has Office ID
        $calender_link = get_field('pcx_office_calender',$user_office_id);
        $calender_iframe = "<iframe src='$calender_link&mode=MONTH&bgcolor=%23ffffff&color=%23AA5A00&showTitle=1&showDescription=0&showNav=0&showDate=1&showPrint=0&showTabs=0&showCalendars=0&showTz=0' style='border-top:solid 1px #DEDEDE' width='100%' height='600' frameborder='0' scrolling='yes'></iframe>";
        echo $calender_iframe;
    }else{ // Otherwise
        echo 'No Office Linked With Your Profile';
    }
...
// BUSINESS-GUIDE URL V2
add_shortcode( 'business-guide-url', 'business_guide_url' );
function business_guide_url(){
    $user_office_id = get_current_user_office();
    if(!$user_office_id){
        return '';
    }
    $busines_guide_url = get_field('business_guide', $user_office_id);
    return $busines_guide_url;
}

// OFFICE RESOURCES BTN V2
add_shortcode( 'office-resources-btn-v2', 'office_resources_btn_v2' );
function office_resources_btn_v2(){
    $user_office_id = get_current_user_office();
    $gd_ids = get_field('office_google_drive_ids',$user_office_id);
    $gd_id = $gd_ids['office_resources'];
    if(!$user_office_id){
...
// Get Coach fields V2
add_shortcode( 'get-coach-fields', 'get_coach_fields' );
function get_coach_fields($atts){
    $args = shortcode_atts( array(
        'field' => null
    ), $atts);
    $user_office_id = get_current_user_office();
...
// Get team member info V2
add_shortcode( 'get-team-member-info', 'get_team_member_info' );
function get_team_member_info($atts){
    $args = shortcode_atts( array(
        'team_title' => null,
        'field' => null
    ), $atts);
    ...
    $field = $args['field'];
    $value = $team_member[$field];
    
    return $value;
}

// Steps Shortcode V2
add_shortcode( 'pcx-steps-v2', 'get_office_step' );
function get_office_step($atts) {
    $args = shortcode_atts(array(
        'step' => null,
        'title' => "false"
    ), $atts);
    ...
    if ($user_office_id) {
        ...
        if ($steps) {
            $result = $steps['step_' . $requested_step];
        }

        if ($requestTitle === "true") {
            $result = $steps['step_' . $requested_step . '_title'];
        }
    }
...
add_shortcode( 'marketing-templates-pdf-v2', 'render_mtp_v2' );
function render_mtp_v2(){
    $user_office_id = get_current_user_office();
    $marketing_file = 'https://drive.google.com/file/d/16cusQlYTlCcwHud3-rbQOjOTKVAVyTGv/preview';
    $is_pdf = false;
    if($user_office_id){
    ...
    ob_start(); 
    if($is_pdf){
        echo "<iframe src='$marketing_file' width='100%' height='900' allow='autoplay'></iframe>";
    }else{
        echo "<iframe src='https://drive.google.com/file/d/16cusQlYTlCcwHud3-rbQOjOTKVAVyTGv/preview' width='100%' height='900' allow='autoplay'></iframe>";
    }
     return ob_get_clean();
}

// Shortcode to get placeholder videos by office ID
add_shortcode( 'render-placeholder-videos-by-day', 'render_placeholder_videos_by_day' );
function render_placeholder_videos_by_day($atts) {
    $args = shortcode_atts(array(
        'day' => null,
    ), $atts);
    ...
    if (!$user_office_id) return 'Invalid Office ID';
    
    $day = $args['day'];

    $placeholder_videos = get_field('videos-repeater-day' . $day, $user_office_id);
    $videos = array();

    if (!empty($placeholder_videos)) {
        foreach ($placeholder_videos as $video) {
            $title = $video['topic-title'];
            $link = $video['topic-link'];
        ...
        ob_start();
        ?>
        <div class="placeholder-videos">
            <ul class="placeholder-videos-list" style="list-style: none;padding:0;">
                <?php foreach ($videos as $video) { ?>
                    <li>
                        <h3 style="margin-bottom: 17px;"><?php echo $video['title']; ?></h3>
                        <?php
                        if ($video['ytId']) {
                            ?>
                            <iframe width="560" height="315" src="https://www.youtube.com/embed/<?php echo $video['ytId']; ?>" title="<?php echo $video['title']; ?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            <?php
                        } else {
                            if (strlen($video['link']) > 0) {
                                ?>
                                <a style="text-decoration:underline;color: blue;" href="<?php echo $video['link']; ?>" target="_blank">Watch Video</a>
                                <?php
                            }
                        }
                        ?>
        ...
        return 'This placeholder is blank. Mark it complete and move to the next video.';
    }
}
...
// Shortcode to render contract writing videos
add_shortcode( 'render-contract-writing-videos', 'render_contract_writing_videos' );
function render_contract_writing_videos($atts) {
    $user_office_id = get_current_user_office();

    if (!$user_office_id) return 'Invalid Office ID';

    $videos = get_field('video-playlist', $user_office_id);

    if (empty($videos)) return 'No videos found';
        ...
        <ul class="contract-writing-videos-list" style="list-style: none;padding:0;">
            ...
            ?>
                <li>
                    ...
                    if ($yt_video) {
                        ?>
                        <iframe width="100%" height="315" src="https://www.youtube.com/embed/<?php echo $yt_video; ?>" title="<?php echo $video['video-title']; ?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                        <?php
                    } else {
                        ?>
                        <a class="vide-link" style="text-decoration:underline;color: blue;" href="<?php echo $video['video-link']; ?>" target="_blank">Watch Video</a>
                        <?php
                    }
                    ?>
                    <?php
                        if ($video['video-title']) {
                            ?>
                            <span class="video-title"><?php echo $video['video-title']; ?></span>
                            <?php
                        }
                    ?>
                    <?php
                        if ($video['video-description']) {
                            ?>
                            <div class="video-description"><?php echo $video['video-description']; ?></div>
                            <?php
                        }
                    ?>
                </li>
...
// Shortcode to check if the office has Essentials Live Call Recordings enabled
add_shortcode( 'essentials-live-enabled', 'essentials_live_enabled' );
function essentials_live_enabled(){
    $user_office_id = get_current_user_office();
    if($user_office_id){
        $is_visible = get_field('display_essentials_live_call_recordings', $user_office_id);
        if($is_visible){
...
// Get topics underneath a specific section
function learndash_filtered_topics_by_section_shortcode( $atts ) {
    $atts_defaults = array(
        'course_id' => 0,
        'section_ids' => '',
    );
                ...
                foreach ( $section->steps as $lesson_id ) {
                    ...
                    if ( ! empty( $topics ) ) {
                        foreach ( $topics as $topic ) {
                            ...
                            $output .= '<li><a href="' . esc_url( $topic_permalink ) . '">' . esc_html( $topic_title ) . '</a></li>';
                        }
                    }
                }
            }
...
add_shortcode( 'learndash_filtered_topics', 'learndash_filtered_topics_by_section_shortcode' );

// Enrolled user into a course
/**
 * Send a webhook when a user is enrolled in a course.
 *
 * @param int $user_id The WordPress user ID.
 * @param int $course_id The LearnDash course ID.
 * @param array $access_list The access list of courses for the user.
 */
function send_webhook_on_user_enrolled( $user_id, $course_id, $access_list ) {
    // Get user data
    ...
    $webhook_url = 'https://2dbfj1mr-3001.usw3.devtunnels.ms/webhook/learndash/enroll-curse';

    // Prepare the data to be sent to the webhook
    $body = array(
        'email'      => $user_email,
        'wordpress_id'    => $user_wp_id,
    ...
    // Optionally, check the response for debugging
...
add_action( 'learndash_update_course_access', 'send_webhook_on_user_enrolled', 10, 3 );
